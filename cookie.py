...
class CookieSpider(object):
    ...
    def _need_update(self):
        ...
        with open('time.txt', 'r') as f:
            data = f.read()
        if not data or (now-float(data)) > 24*60*60: return True
        return False
    ...
    def get_cookies(self, cookie_q):
        try:
            ...
            print 'time.txt 文件不存在，需要更新cookie'
    ...
    def get_cookie(self, user, cookie_q):
        ...
        print 'getcookie:' + str(item)
        cookie_q.put(item)
        print 'put cookie to q'
    ...
    def login(self, session, headers, user):
        ...
        print 'login得到的cookies:' + str(session.cookies.get_dict())
        print '从login中得到的url:\n' + url
        # 访问该网址得到随后的第一次登录网址login1
        res = session.get(url, headers=headers, verify=False)
        print '经过crossdomain后，cookies变为:' + str(session.cookies.get_dict())
        ...
        print 'login1_res' + res1.content + '\n'
        print 'login1得到的cookies:' + str(session.cookies.get_dict())
        ...
        print 'login2_res' + res2.content + '\n'
        print 'login2得到的cookies:' + str(session.cookies.get_dict())
